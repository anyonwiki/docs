...
For 1. we decided to use the canonical ordering introduced in [this paper](https://pubs.aip.org/aip/jmp/article/64/9/091703/2912575/On-low-rank-fusion-rings). For 2, a low-level Mathematica algorithm was written to construct the kernel of a matrix. The file used to sort and group the categories into 3 levels, determining the indices $i,j,k$ can be found [here](https://github.com/gert-vercleyen/Anyonica/blob/develop/NamingCategories.wl).  The algorithm is not optimized for speed since we wanted it to be easy to implement in
